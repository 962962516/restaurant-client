...
watch(()=> props.allCombo, (newValue => {
  comboData.value = props.allCombo
...
watch(searchProduct, (newValue) => {
  ...
  // 发送API请求
  axios.get(`/admin/searchCombo?productName=${searchProductRequest}`)
      .then(response => {
...
const handleSelect = (item) => {
  ...
  console.log('123',comboData.value)
};
...
function submit(item){
  requestData.comboId = item.comboId
  axios.post('/enterprise/reviseComboPrice',requestData)
      .then(response => {
